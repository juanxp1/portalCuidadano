(function () {
  ...
  L.Routing.TomTom = L.Class.extend({
    options: {
      ...
      language: "es-MX", // en-GB
      instructionsType: "text", // coded, text, tagged
      traffic: true,
          ...
          if (!timedOut) {
            if (!err) {
              ...
              callback.call(context || callback, {
                ...
                message:
                  "HTTP request failed: " +
                  JSON.parse(err.responseText).error.description,
              });
            }
          }
        }, this)
    ...
    buildRouteUrl: function (waypoints, options) {
      ...
      if (_options.avoid == "" || _options.avoid == []) delete _options.avoid;

      if (_options.instructionsType == "") delete _options.instructionsType;

      if (_options.language == "") delete _options.language;

      if (
        _options.departAt.match(
      ...
      return (
        ...
        Object.keys(_options)
          .map(function (k) {
            return (
              encodeURIComponent(k) + "=" + encodeURIComponent(_options[k])
            );
          })
          .join("&")
      );
    ...
    _convertSummary: function (summaries) {
      var result = { totalDistance: 0, totalTime: 0 },
        i;
