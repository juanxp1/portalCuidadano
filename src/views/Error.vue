  ...
  <v-main>
        ...
        <v-col cols="12" md="6" sm="12">
          <div class="denied__wrapper">
            ...
            <router-link :to="{ name: 'Home' }">
              <i18n tag="button" class="denied__link" path="router.home" />
            </router-link>
            <svg
              id="astronaut"
    ...
    <template v-if="$vuetify.breakpoint.mdAndUp">
      <audio
        ...
        style="display: none"
      >
        <source :src="require('@/util/game/EvilCircles.wav')" />
      </audio>
      <audio
        ref="explode"
        id="explode"
        ...
        style="display: none"
      >
        <source :src="require('@/util/game/explode.wav')" />
      </audio>
      <audio
        ref="power"
        id="power"
        ...
        style="display: none"
      >
        <source :src="require('@/util/game/power.wav')" />
      </audio>
      <audio
        ref="shoot"
        :autoplay="false"
        ...
        style="display: none"
      >
        <source :src="require('@/util/game/shoot.mp3')" />
      </audio>
    </template>
  </v-main>
</template>
...
import VP5 from "@/components/app/VP5";
import { Enemy } from "@/util/game/Enemy";
import { Animation } from "@/util/game/Animations";
import { Life } from "@/util/game/Life";
import { PowerUp } from "@/util/game/PowerUp";
import { Ship } from "@/util/game/Ship";
...
export default {
  ...
  props: {
    ...
    text: {
      type: [String],
      default: "Hmm, parece que esta página no existe.",
    },
  },
  data: () => ({
    rate: STARTING_RATE,
    ...
    font: null,
    gameOver: false,
    play: require("@/util/game/audio.png"),
    mute: require("@/util/game/mute.png"),
    restartImg: require("@/util/game/restart.png"),
    pausedByUser: false,
  }),
  mounted() {
  ...
  methods: {
    preload(sketch) {
      this.font = sketch.loadFont(require("@/util/game/Dosis-SemiBold.ttf"));
    },
    setup(sketch) {
      ...
      this.musicBtn = sketch.createImg(
        this.$refs.music && this.$refs.music.paused ? this.mute : this.play,
        "Reproducir/Pausar Música"
      );
      this.musicBtn.position(width, 10);
      this.musicBtn.mousePressed(this.toggleMusic);
      const restartBtn = sketch.createImg(this.restartImg, "Reiniciar Juego");
      restartBtn.position(width - 35, 10);
      const that = this;
      restartBtn.mousePressed(function () {
        that.endGame(sketch, true);
      });
      this.ship = new Ship(sketch);
      this.intro(sketch);
    },
    intro(sketch) {
      sketch.noLoop();
      this.ship.position.x = -50;
      this.ship.position.y = sketch.height - 250;
      this.lives = [];
      this.enemies = [];
      this.shots = [];
      this.powerups = [];
      ...
      clearTimeout(this.level1Timer);
      clearTimeout(this.level2Timer);
      clearTimeout(this.level3Timer);
      this.level = 0;
      this.score = 0;
      this.rate = 0;
      this.gameOver = false;
      ...
      setTimeout(function () {
        that.ship.position.x = -50;
        // sketch, ship, shots, animations, type
        that.enemies.push(
          new Enemy(sketch, that.ship, that.shots, that.animations, 0)
        );
        that.setIntroText(sketch, "3");
      }, 658);

      setTimeout(function () {
        that.setIntroText(sketch, "2");
      }, 1316);
      ...
      setTimeout(function () {
        that.setIntroText(sketch, "¡Vamos!");
      }, 2632);

      setTimeout(function () {
        that.setIntroText(sketch, "");
        that.restart(sketch);
      }, 3290);
    },
    setIntroText(sketch, text) {
      sketch.clear();
      sketch.fill("#ffffff");
    ...
    setHighScore(sketch, prefix = "Puntuación más alta:") {
      sketch.fill("#ffffff");
      sketch.textFont(this.font);
      sketch.textAlign(sketch.LEFT);
      sketch.textSize(14);
      sketch.text(`${prefix} ${this.highscore}`, 1, 50);
    },
    setGameOver(sketch) {
      sketch.fill("#ffffff");
      sketch.textFont(this.font);
    ...
    restart(sketch) {
      if (this.$refs.music && this.$refs.music.paused && !this.pausedByUser) {
        this.playMusic();
      }
      this.gameOver = false;
      this.score = 0;
      this.ship.position.x = sketch.width / 2;
      this.ship.position.y = sketch.height - 250;
      this.lives = [];
      this.enemies = [];
      this.shots = [];
      ...
      for (let i = 0; i < STARTING_LIVES; i++) {
        this.lives.push(new Life(sketch, i));
      }
      this.level = 0;
      this.score = 0;
      if (this.highscore > 0) {
        this.setHighScore(sketch);
      }
      this.rate = STARTING_RATE;
      const that = this;
      this.rateInterval = setInterval(function () {
        that.rate -= RATE_INCREMENT;
      }, 1000);
      this.powerupInterval = setInterval(function () {
        that.powerups.push(new PowerUp(sketch, sketch.round(sketch.random())));
      }, 20000);
      this.level1Timer = setTimeout(function () {
        that.level++;
      }, 13000);
      this.level2Timer = setTimeout(function () {
        that.level++;
      }, 25000);
      this.level3Timer = setTimeout(function () {
        that.level++;
      }, 35000);
      this.spawn(sketch);
      sketch.loop();
    ...
    spawn(sketch) {
      ...
      this.enemies.push(
        new Enemy(
          ...
          this.shots,
          this.animations,
          sketch.round(sketch.random(this.level))
        )
      );
      this.spawnTimer = setTimeout(function () {
        that.spawn(sketch);
      }, this.rate);
      if (this.rate < 300) clearInterval(this.rateInterval);
    },
    draw(sketch) {
      sketch.clear();
      sketch.fill("#ffffff");
      ...
      if (this.gameOver) {
        this.setGameOver(sketch);
      }
      // update and delete enemies
      this.updateEnemies(sketch);
      // update and delete shots
      this.updateShots(sketch);
      // handle power ups
      this.updatePowerUps(sketch);
      // update and delete animations
      this.updateAnimations();
      for (let elt of this.lives) {
        elt.show();
      }
      // update ship
      this.ship.show();
      ...
      for (let damage of this.animations) {
    ...
    updateEnemies(sketch) {
      ...
      for (let i = this.enemies.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          sketch.height + this.enemies[i].radius
        ) {
          this.enemies.splice(i, 1);
          this.lives.splice(this.lives.length - 1);
          if (this.lives.length === 0) this.endGame(sketch);
        } else if (
          sketch.dist(
            this.enemies[i].position.x,
            this.enemies[i].position.y,
            this.ship.position.x,
            this.ship.position.y + SHIP_LENGTH / 2
          ) < this.enemies[i].radius
        ) {
          this.animations.push(
            new Animation(sketch, this.ship, this.enemies[i], "death")
          );
          this.enemies.splice(i, 1);
          this.lives.splice(this.lives.length - 1);
          if (this.lives.length === 0) this.endGame(sketch);
    ...
    updateShots(sketch) {
      ...
      for (let i = this.shots.length - 1; i >= 0; i--) {
        ...
        for (let j = this.enemies.length - 1; j >= 0; j--) {
          ...
          if (this.shots[i].side === "player") {
            // if this player's shot is touching an enemy, then...
            if (
              sketch.dist(
                this.enemies[j].position.x,
                this.enemies[j].position.y,
                this.shots[i].position.x,
                this.shots[i].position.y
              ) < this.enemies[j].radius
            ) {
              ...
              if (this.enemies[j].health === 0) {
                ...
                this.animations.push(
                  new Animation(sketch, this.ship, this.enemies[j], "death")
                );
                this.score += this.enemies[j].type * 30 + 50;
                this.enemies.splice(j, 1);
                // if the enemy is hurt but not destroyed, then...
              } else {
                // play the damage animation
                this.animations.push(
                  new Animation(sketch, this.ship, this.enemies[j], "damage")
                );
              }
        ...
        if (
          this.shots[i] &&
          (this.shots[i].position.y < 0 ||
            this.shots[i].position.y > sketch.height ||
            this.shots[i].position.x > sketch.width ||
            this.shots[i].position.x < 0)
        ) {
          this.shots.splice(i, 1);
        }
      }
    ...
    updatePowerUps(sketch) {
      ...
      for (let i = this.powerups.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          sketch.height + this.powerups[i].length
        ) {
          ...
          this.ship.radius + (2 * this.powerups[i].length) / 3
        ) {
          this.animations.push(
            new Animation(sketch, that.ship, that.ship, "powerupStart")
          );
          setTimeout(function () {
            that.animations.push(
              new Animation(sketch, that.ship, that.ship, "powerup")
            );
          }, 500);
          if (this.powerups[i].type === 0) {
            this.ship.shotRate = 180;
            setTimeout(function () {
              that.ship.shotRate = 220;
            }, POWERUP_TIME);
    ...
    updateAnimations() {
      for (let i = this.animations.length - 1; i >= 0; i--) {
        ...
        if (this.animations[i].type === "death") {
          if (this.animations[i].timer >= 43) {
            this.animations.splice(i, 1);
          }
        } else if (
          this.animations[i].type === "damage" ||
          this.animations[i].type === "playerDamage"
        ) {
          if (this.animations[i].timer >= 15) {
            this.animations.splice(i, 1);
          }
        } else if (this.animations[i].type === "playerDeath") {
          if (this.animations[i].timer >= 30) {
            this.animations.splice(i, 1);
          }
        } else if (
          this.animations[i].type === "teleportIn" ||
          this.animations[i].type === "teleportOut"
        ) {
          if (this.animations[i].timer >= 30) {
            this.animations.splice(i, 1);
          }
        } else if (this.animations[i].type === "powerupStart") {
          if (this.animations[i].timer >= 30) {
            this.animations.splice(i, 1);
          }
        } else if (this.animations[i].type === "powerup") {
          if (this.animations[i].timer >= 360) {
            this.animations.splice(i, 1);
          }
        }
      }
    ...
    addLife(sketch) {
      this.lives.push(new Life(sketch, this.lives.length));
    },
    endGame(sketch, isClicked = false) {
      ...
      if (!isClicked) {
        this.animations.push(
          new Animation(sketch, new Ship(sketch), this.ship, "playerDeath")
        );
      }
      const that = this;
      setTimeout(function () {
        ...
        that.shots = [];
        that.powerups = [];
        that.ship.position.x = sketch.width / 2;
        that.ship.position.y = sketch.height - 250;
      }, 420);
      setTimeout(function () {
        that.shots = [];
        that.animations = [];
    ...
    keyPressed(sketch) {
      switch (sketch.keyCode) {
        case 32:
          this.ship.shoot(this.ship, this.shots);
          return this.$refs.shoot && this.$refs.shoot.play();
        case 82:
          if (this.rate === STARTING_RATE) this.intro(sketch);
          break;
      }
    },
    playMusic() {
      this.pausedByUser = false;
      this.musicBtn.elt.src = this.play;
      if (this.$refs.music) {
        this.$refs.music.play();
      }
    },
    pauseMusic() {
      this.musicBtn.elt.src = this.mute;
    ...
    toggleMusic() {
      if (this.$refs.music && this.$refs.music.paused) {
        this.playMusic();
      } else {
        this.pauseMusic();
      }
  ...
  computed: {
    errorText() {
      return (this.text || "").length > 230
        ? `${(this.text || "").substring(0, 227)}...`
        : this.text;
    },
  },
};
